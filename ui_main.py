...
class MainWindow(QMainWindow):
    def __init__(self, store, engine):
        ...
        # =========================
        # 3.5) Config Editor (Load Config -> Edit)
        # =========================
    ...
    def on_cload_clicked(self):
        path, _ = QFileDialog.getOpenFileName(
            ...
            "Load Config",
            "",
            "Config Files (*.json *.yaml *.yml *.ini);;All Files (*)"
        )
        if not path:
            return

        # ✅ 선택된 파일을 "편집 모드"로 열기
        self.open_config_editor(path)

        # (기존 동작 유지) 프로젝트에 load_config가 있으면 연결
    ...
    def _build_config_editor(self):
        """Internal text editor dialog for config files (json/yaml/ini)."""
        self.config_editor_dlg = QDialog(self)
        self.config_editor_dlg.setWindowTitle("Config Editor")
        self.config_editor_dlg.resize(900, 650)

        layout = QVBoxLayout(self.config_editor_dlg)
        ...
        self.txt_config = QPlainTextEdit()
        # nicer tabs for config editing
        try:
            self.txt_config.setTabStopDistance(
                4 * self.txt_config.fontMetrics().horizontalAdvance(" ")
            )
        except Exception:
            pass
        layout.addWidget(self.txt_config, 1)

        btn_row = QHBoxLayout()
        self.btn_cfg_save = QPushButton("Save")
        self.btn_cfg_save_as = QPushButton("Save As...")
        self.btn_cfg_apply = QPushButton("Apply/Reload")
        self.btn_cfg_close = QPushButton("Close")
        btn_row.addWidget(self.btn_cfg_save)
        btn_row.addWidget(self.btn_cfg_save_as)
        btn_row.addWidget(self.btn_cfg_apply)
        btn_row.addStretch(1)
        btn_row.addWidget(self.btn_cfg_close)
        layout.addLayout(btn_row)

        self.btn_cfg_save.clicked.connect(self._config_save)
        self.btn_cfg_save_as.clicked.connect(self._config_save_as)
        self.btn_cfg_apply.clicked.connect(self._config_apply)
        self.btn_cfg_close.clicked.connect(self.config_editor_dlg.close)

    def open_config_editor(self, path: str):
        p = Path(path)
        if not p.exists():
            QMessageBox.warning(self, "Config", f"File not found:\n{p}")
            return
        try:
            text = p.read_text(encoding="utf-8", errors="replace")
        except Exception as e:
            QMessageBox.critical(self, "Config", f"Read failed:\n{e}")
            return

        self.current_config_path = str(p)
        self.lbl_config_path.setText(f"Editing: {self.current_config_path}")
        self.txt_config.setPlainText(text)

        self.config_editor_dlg.show()
        self.config_editor_dlg.raise_()
        self.config_editor_dlg.activateWindow()

    def _config_write_to(self, path: str) -> bool:
        try:
            Path(path).write_text(self.txt_config.toPlainText(), encoding="utf-8")
            return True
        except Exception as e:
            QMessageBox.critical(self, "Config", f"Save failed:\n{e}")
            return False

    def _config_save(self):
        if not getattr(self, "current_config_path", None):
            self._config_save_as()
            return
        if self._config_write_to(self.current_config_path):
            try:
    ...
    def _config_save_as(self):
        start = self.current_config_path or str(Path.cwd())
    ...
    def _config_apply(self):
        ...
        # Apply = (1) save current editor content to file, (2) engine.load_config(), (3) refresh UI
        if not self._config_write_to(self.current_config_path):
            return

        if hasattr(self.engine, "load_config"):
            try:
                self.engine.load_config(self.current_config_path)
            except Exception as e:
                QMessageBox.critical(self, "Config Apply Failed", str(e))
                return

        self.refresh_all()
        try:
            self.statusBar().showMessage(f"Loaded: {self.current_config_path}", 3000)
        except Exception:
            pass
